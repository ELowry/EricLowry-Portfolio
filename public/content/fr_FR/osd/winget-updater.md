...
WinGet Updater est une passerelle qui simplifie l'utilisation de WinGet pour les mises à jour automatiques et manuelles. Le script propose de définir quelles applications sont automatiquement ou manuellement mises à jour ou bien totalement ignorées. Il peut être configuré pour fonctionner entièrement en arrière-plan, sans déranger l'utilisateur·rice, ou lancé manuellement à partir du menu Démarrer.
...
J'ai voulu garder cet outil aussi simple et direct que possible et j'ai donc choisi de rester sur un applicatif écrit entièrement en PowerShell, avec un script dédié pour son installation et sa mise à jour. J'ai ainsi pu facilement le distribuer avec un installateur Windows standard (généré avec Inno Setup), ainsi qu'une version portable qui peut être lancée avec un simple script `.bat`.
...
Comme il s'agit d'un script PowerShell, il est aussi possible de le lancer directement en ligne de commande, sans installation. Des conditions d'exécution sont disponibles sous cette forme, telles que `-Silent` pour outrepasser la fenêtre de contrôle, ou `-Minimal` pour réduire les détails d'exécution dans la console.

### Pratique pour la maintenance à distance

L'un de mes objectifs avec cet outil était la gestion des ordinateurs de mes connaissances et des membres de ma famille moins technophiles. Le mode silencieux a été ajouté spécifiquement pour cet usage:  
